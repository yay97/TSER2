...
deep_learning_models = ["fcn", "resnet", "inception","mlp","mcdcnn","encoder","mcnn"]
tsc_models = ["rocket"]
...
def create_regressor(regressor_name, input_shape, output_directory, verbose=1, itr=1):
    ...
    if regressor_name == "mlp":
        ...
        return mlp.MLPRegressor(output_directory,input_shape, verbose)
    if regressor_name == "mcdcnn":
        from models.deep_learning import mcdcnn
        return mcdcnn.MCDCNNRegressor(output_directory,input_shape, verbose)
    if regressor_name == "encoder":
        from models.deep_learning import encoder
        return encoder.ENCODERRegressor(output_directory,input_shape, verbose)
    if regressor_name == "mcnn":
        from models.deep_learning import MCNN
        return MCNN.MCNNRegressor(output_directory,input_shape, verbose)
